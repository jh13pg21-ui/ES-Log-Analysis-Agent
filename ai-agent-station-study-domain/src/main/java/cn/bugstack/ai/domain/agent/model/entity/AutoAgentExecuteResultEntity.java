...
public class AutoAgentExecuteResultEntity {
     ...
     * 创建意图识别结果（Step0 专用）
     */
    public static AutoAgentExecuteResultEntity createIntentResult(String intentType, boolean needsRag, boolean needsMcp, String reasoning, String sessionId) {
        String content = String.format("意图类型: %s | 知识库: %s | 工具: %s | 理由: %s",
                intentType, needsRag ? "需要" : "不需要", needsMcp ? "需要" : "不需要", reasoning);
        return AutoAgentExecuteResultEntity.builder()
                .type("intent")
                .subType(intentType.toLowerCase())
                .step(0)
                .content(content)
                .completed(false)
                .timestamp(System.currentTimeMillis())
