...
public class Step4LogExecutionSummaryNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        try {
            ...
            // 记录执行总结
            logExecutionSummary(dynamicContext.getMaxStep(), dynamicContext.getFullHistory(), dynamicContext.isCompleted());
            ...
            metricsRegistry.recordStepDuration(requestParameter.getSessionId(), "Step4_LogExecutionSummary",
                    System.nanoTime() - startNs);
            // 在收尾处打印本 session 的指标快照，便于离线统计
            try {
                Object snapshot = metricsRegistry.snapshotSession(requestParameter.getSessionId());
                log.info("\n📈 === Session 指标快照 sessionId={} ===\n{}",
                        requestParameter.getSessionId(),
                        com.alibaba.fastjson.JSON.toJSONString(snapshot, true));
            } catch (Exception e) {
                log.warn("打印 session 指标快照失败: {}", e.getMessage());
            }
        }
     ...
     * 记录执行总结
     */
    private void logExecutionSummary(int maxSteps, String executionHistory, boolean isCompleted) {
        log.info("\n📊 === 动态多轮执行总结 ====");

        int actualSteps = Math.min(maxSteps, executionHistory.split("=== 第").length - 1);
