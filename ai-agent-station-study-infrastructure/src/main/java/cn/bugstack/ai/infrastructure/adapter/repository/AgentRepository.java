    ...
    public List<AiClientAdvisorVO> AiClientAdvisorVOByClientIds(List<String> clientIdList) {
        ...
        for (String clientId : clientIdList) {
            ...
            for (AiClientConfig config : configs) {
                ...
                AiClientAdvisorVO.MemoryAnswer memoryAnswer = null;
                AiClientAdvisorVO.SummaryBufferMemory summaryBufferMemory = null;
                ...
                if (extParam != null && !extParam.trim().isEmpty()) {
                    try {
                        ...
                        } else if ("MemoryAdvisor".equals(aiClientAdvisor.getAdvisorType())) {
                            memoryAnswer = JSON.parseObject(extParam, AiClientAdvisorVO.MemoryAnswer.class);
                        } else if ("SummaryBufferMemory".equals(aiClientAdvisor.getAdvisorType())) {
                            summaryBufferMemory = JSON.parseObject(extParam, AiClientAdvisorVO.SummaryBufferMemory.class);
                        }
                    } catch (Exception e) {
                        // 解析失败时忽略，使用默认值null
                    }
                ...
                AiClientAdvisorVO advisorVO = AiClientAdvisorVO.builder()
                        ...
                        .summaryBufferMemory(summaryBufferMemory)
                        .build();
